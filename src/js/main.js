...
import { CARDS, emperorTexture, slaveTexture } from "./card";
...
function nextRound() {
  if (CARDS[0].name === "hand playerCard1 emperor") {
    ...
    p1.style.background =
      "linear-gradient(90deg, rgba(223, 207, 96, 0.8) 0%, rgba(255, 255, 255, 0) 100%)";
    p2.style.background =
      "linear-gradient(90deg, rgba(83, 0, 0, 0.8) 0%, rgba(255, 255, 255, 0) 100%)";
...
function resetAndUpdate(side, sideText) {
  ...
  for (let i = 0; i < 5; i++) {
    ...
    opponentCards[i].rotation.set(
      Math.PI * 2,
      Math.PI,
      initialCardsRotations[i + 5]
    );
    opponentCards[i].scale.set(1, 1, 1);

    if (
      CARDS[i].name === "hand playerCard1 slave" ||
      CARDS[i].name === "hand playerCard1 emperor"
    )
      CARDS[i].name = CARDS[i].name;
    else CARDS[i].name = "hand " + CARDS[i].name;
  }
...
window.addEventListener("click", function (e) {
  ...
  if (hoveredCard && playNext && !finished) {
    ...
    tl2
      ...
      .to(
        opponentCards[randomNumber].position,
        {
          y: 3.18,
          z: -0.7,
          x,
      ...
      .to(
        opponentCards[randomNumber].scale,
        {
          x: 1.5,
          y: 1.5,
          z: 1.5,
      ...
      .to(
        opponentCards[randomNumber].rotation,
        {
          y: 0,
          delay: 1,
        },
      ...
      .to(
        opponentCards[randomNumber].position,
        {
          ...
          onComplete: function () {
            ...
            if (
              hoveredCName.includes("emperor") &&
              oppCName.includes("citizen")
            )
            ...
            if (hoveredCName.includes("slave") && oppCName.includes("citizen"))
              resetAndUpdate("opponent", oScore);
            if (
              hoveredCName.includes("citizen") &&
              oppCName.includes("emperor")
            )
              resetAndUpdate("opponent", oScore);
          },
        },
